...
export class AddressRepository {
  ...
  async listRootAdminUnits(countryId: string): Promise<AdministrativeUnit[]> {
    return db
      ...
      .where(and(eq(administrativeUnits.countryId, countryId), isNull(administrativeUnits.parentId)))
  ...
  async getAdministrativeUnitById(adminUnitId: string): Promise<AdministrativeUnit | null> {
    const [unit] = await db
      .select()
      .from(administrativeUnits)
  ...
  async searchAdministrativeUnits(params: {
    ...
    const [units, totalResult] = await Promise.all([
      db
        ...
        .limit(params.limit)
        .offset(offset),
      db.select({ value: count() }).from(administrativeUnits).where(whereClause),
    ]);
  ...
  async listUserLocationsByUserId(
    ...
    const [locations, totalResult] = await Promise.all([
      db
        ...
        .limit(limit)
        .offset(offset),
      db.select({ value: count() }).from(userLocations).where(eq(userLocations.userId, userId)),
    ]);
  ...
  async getUserLocationById(userId: string, locationId: string): Promise<UserLocation | null> {
    const [location] = await db
      .select()
      .from(userLocations)
      .where(and(eq(userLocations.userId, userId), eq(userLocations.id, locationId)))
      .limit(1);
  ...
  async updateUserLocationById(
    ...
    data: Partial<
      Pick<
        NewUserLocation,
        "adminUnitId" | "route" | "streetNumber" | "formattedAddress" | "latitude" | "longitude"
      >
    >,
  ...
  async deleteUserLocationById(userId: string, locationId: string): Promise<boolean> {
